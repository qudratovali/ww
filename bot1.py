...
BOT_TOKEN = os.environ.get("BOT_TOKEN", "8635279428:AAErm3o4ICNPiN0iwS3VtvEwSFJpMgAYGR4")
...
async def username_handler(client, message: Message):
    ...
    try:
        ...
        # Hikoyalar bormi tekshirish — userbot orqali
        if userbot:
            story_count = 0
            try:
                async with userbot:
                    async for _ in userbot.get_stories(user_id):
                        story_count += 1
            except Exception:
                pass
...
@bot.on_callback_query(filters.regex(r"^stories_(.+)$"))
async def download_stories_cb(client, callback_query):
    ...
    if not userbot:
        await callback_query.message.reply_text("❌ Hikoya yuklab olish xizmati mavjud emas.")
        return

    try:
        stories = []
        async with userbot:
            async for story in userbot.get_stories(user_id):
