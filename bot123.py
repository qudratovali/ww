...
BOT_TOKEN = os.environ.get("BOT_TOKEN", "8635279428:AAEi_e57YCOsUsfYKY7Nnx_k9PKkVURwgBc")
...
async def username_handler(client, message: Message):
    ...
    try:
        ...
        uid = user.id
        fname = ((user.first_name or "") + " " + (user.last_name or "")).strip() or "—"
        uname = "@" + user.username if user.username else "—"
        bio = getattr(user, "bio", getattr(user, "about", "—")) or "—"

        info = (
            "<b>Foydalanuvchi malumotlari</b>\n\n"
            "Ism: <b>" + fname + "</b>\n"
            "Username: <b>" + uname + "</b>\n"
            "ID: <code>" + str(uid) + "</code>\n"
            "Bio: " + bio + "\n\n"
            "Bot: " + ("Ha" if user.is_bot else "Yoq") + "\n"
            "Premium: " + ("Ha" if user.is_premium else "Yoq") + "\n"
        ...
        if userbot:
            ...
            if story_count > 0:
                ...
                await message.reply_text(str(story_count) + " ta aktiv hikoya bor!", reply_markup=kb)
...
async def download_stories_cb(client, callback_query):
    ...
    if not userbot:
        await client.send_message(chat_id, "❌ Mavjud emas.")
        return

    try:
        log.info("Hikoyalar yuklanmoqda: " + str(uid))
        stories = []
        async for story in userbot.get_stories(uid):
            stories.append(story)
            log.info("Story topildi: " + str(i if False else len(stories)))

        if not stories:
            await client.send_message(chat_id, "📭 Hikoyalar topilmadi.")
            return

        await client.send_message(chat_id, str(len(stories)) + " ta hikoya yuklanmoqda...")

        for i, story in enumerate(stories, 1):
            try:
                if story.photo:
                    file = await userbot.download_media(story.photo, file_name="/tmp/story_" + str(i) + ".jpg")
                    await client.send_photo(chat_id, file, caption="Hikoya " + str(i) + "/" + str(len(stories)))
                    os.remove(file)
                elif story.video:
                    file = await userbot.download_media(story.video, file_name="/tmp/story_" + str(i) + ".mp4")
                    await client.send_video(chat_id, file, caption="Hikoya " + str(i) + "/" + str(len(stories)))
                    os.remove(file)
                else:
                    log.info("Story " + str(i) + " noma'lum tur")
            except Exception as e:
                log.error("Story " + str(i) + ": " + str(e))

        await client.send_message(chat_id, "✅ Barcha hikoyalar yuborildi!")
    except Exception as e:
        log.error("Callback xato: " + str(e))
        await client.send_message(chat_id, "❌ Xatolik: " + str(e))
